...
app.innerHTML = /* html */ `
  <aside class="sidebar">
    <div class="brand">
      ...
      <div>
        <h1>A* по городу</h1>
        <p>радиально-кольцевая сеть — центр Москвы</p>
      </div>
    </div>
    ...
    <section class="card">
      <h2>Карта</h2>
      <div class="btn-col">
        <button class="btn tonal" id="btn-regen">Новый город</button>
...
const city = new CityMap(canvas, { rings: 8, spokes: 14 });
...
city.onSearch = (s: CitySearchState) => {
  ...
  el.cost.textContent = s.pathCost > 0 ? `${Math.round(s.pathCost)} усл.ед.` : "—";
  if (s.running) {
...
document
  ...
  .addEventListener("click", () => city.regenerate({ rings: 8, spokes: 14 }, true));
